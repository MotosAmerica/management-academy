// ==========================================================================
// Motos America Management Academy — Application logic
...
(function () {
  ...
  // <strong>/<em> tags safely (Management Academy content uses italic quotes
  // in its Weak/Strong word tracks, same as other MAU academies).
  ...
  function roleLabel(role) {
    return ROLE_LABELS[role] || role || 'Department Manager';
  }
  ...
  function renderLogin(root) {
    ...
    card.appendChild(el('div', { class: 'login-card__sub' }, ['Management Academy']));
    ...
    const roleSelect = el('select', { id: 'login-role' }, [
      el('option', { value: 'sales_manager' }, ['Sales Manager']),
      el('option', { value: 'service_manager' }, ['Service Manager']),
      el('option', { value: 'parts_manager' }, ['Parts Manager']),
      el('option', { value: 'fi_manager' }, ['F&I Manager']),
      el('option', { value: 'gm' }, ['General Manager']),
    ]);
    ...
    // MA Corporate is General-Manager-only: lock the role dropdown to GM and
    ...
    async function handleSubmit() {
      ...
      // always General Manager, even if the role field were tampered with.
  ...
  const ACADEMY = 'management_academy';
  ...
  async function findOrCreateTrainee(name, store, role) {
    ...
    try {
      ...
      // Service, Parts, and F&I rows for people with the same name/store.
  ...
  function renderTopbar(root, trainee) {
    ...
    const brand = el('div', { class: 'topbar__brand', onclick: () => navigate({ view: 'toc' }) }, [
      ...
      el('div', { class: 'topbar__brand-sub' }, ['Management Academy']),
    ]);
    ...
    if (trainee.role === 'gm' || trainee.role === 'admin') {
  ...
  function buildModuleMenuPanel() {
    ...
    panel.appendChild(buildGroup('Part I \u2014 Leading the Department', partI));
    panel.appendChild(
      el('button', { class: 'module-menu__item module-menu__item--exam', onclick: () => goTo({ view: 'exam', part: 'I' }) }, ['Part I Exam'])
    );
    panel.appendChild(buildGroup('Part II \u2014 Advanced Leadership & the GM', partII));
    panel.appendChild(
  ...
  async function renderTOC(root, trainee) {
    ...
    page.appendChild(buildPartSection('I', 'Leading the Department', partI, 'part1-exam', 'Part I Exam \u2014 Leading the Department', DATA.part1Exam));
    page.appendChild(buildPartSection('II', 'Advanced Leadership & the General Manager', partII, 'part2-exam', 'Part II Exam \u2014 Advanced Leadership & the GM', DATA.part2Exam));
  }
  ...
  function renderQuizOrExam(root, trainee, opts) {
    ...
    if (opts.mode === 'quiz') {
      ...
      quizLabel = isOne ? 'Part I Exam \u2014 Leading the Department' : 'Part II Exam \u2014 Advanced Leadership & the GM';
      backRoute = { view: 'toc' };
      eyebrow = `Part ${opts.part} Exam`;
      title = isOne ? 'Leading the Department' : 'Advanced Leadership & the General Manager';
  ...
  async function renderReport(root, trainee) {
    ...
    if (trainee.role !== 'gm' && trainee.role !== 'admin') {
      page.appendChild(el('div', { class: 'empty-state' }, ['This report is only available to General Managers.']));
      root.appendChild(page);
      return;
    }

    page.appendChild(el('div', { class: 'hero__eyebrow' }, ['General Manager view']));
    page.appendChild(el('div', { class: 'hero__title' }, ['Training Report']));

    // MA Corporate GMs oversee every dealership, so their report always
    // shows all stores by default (with the option to still filter down).
    // Store-based GMs also start on "All stores" but this comment exists
    ...
    if (isCorporateManager) {
      page.appendChild(el('div', { class: 'banner banner--info' }, [
        'You\u2019re viewing results from every store \u2014 MA Corporate GMs see the full company-wide picture by default. Use the store filter below to narrow to one dealership.',
      ]));
    }
    ...
    const totalQuizzes = totalModuleCount() + 2; // 23 modules + 2 exams
